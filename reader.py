...
class MyReader(bvh.BVHReader):
    ...
    def __init__(self, filename, onTheSpot=False):
        ...
        super(MyReader, self).__init__(filename)
    ...
    def bvh2xyz(self, channels):
        ...
        for i in range(len(self.nodes)):
            ...
            if i == 0:
                ...
                xyzStruct[i].position = thisPosition
                xyzStruct[i].rotation = thisRotation
            else:
                parent = self.nodeOrder[i][1]
                xyzStruct[i].position = (offsets + thisPosition) * xyzStruct[parent].rotation + xyzStruct[
                    parent].position
    ...
    def read(self):
        ...
        limits = self.split()
        return self.points, self.channels, self.nodes, self.nodeOrder, limits
